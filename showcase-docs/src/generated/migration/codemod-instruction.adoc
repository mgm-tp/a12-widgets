...
. *Recipe-based execution* — Run a specific codemod recipe by its identifier
. *Version-based migration* — Run all applicable recipes for a target library version
...
. *Review the changes* — Codemods apply transformations based on pattern matching and may not cover all edge cases. Carefully review the generated diff before committing.
. *Run linters and formatters* — Codemods do not automatically apply code formatting. Run your project's linter (e.g., ESLint) and formatter (e.g., Prettier) to ensure code style consistency.
. *Execute tests* — Run your test suite to verify that the transformations did not introduce regressions.
. *Commit incrementally* — If running multiple recipes or migrating across versions, consider committing after each successful transformation for easier rollback if issues arise.
