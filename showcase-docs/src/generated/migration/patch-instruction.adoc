...
Inside this folder, there are *two versions* of each patch: one in the `+pnpm+` subfolder for native link:https://pnpm.io/[pnpm] patching feature,
...
and `+++++` for those created by other tools like link:https://www.npmjs.com/package/patch-package[patch-package].
...
. Copy the patch from its location inside the Widgets package to the root-level `+patches+` folder (create the folder if it doesn’t exist).
*Example:*
...
----
. Add to your *package.json* located at the root level (the same level as `+pnpm-workspace.yaml+`):
+
[source,json]
----
{
...
. Install the link:https://www.npmjs.com/package/patch-package[patch-package] package as a development dependency:
+
[source,bash]
----
npm install patch-package --save-dev
----
. Add the following script to the project root `+package.json+` file (the one has the same level as `+package-lock.json+`):
+
[source,json]
----
{
