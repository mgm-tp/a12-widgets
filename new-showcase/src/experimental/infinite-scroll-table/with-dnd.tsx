...
import type { BaseColumnType, RowLoadingStatus, TableDragDropOptions } from "@com.mgmtp.a12.widgets/widgets-core";
import { Table, Button, Icon, provider as DeviceDetector } from "@com.mgmtp.a12.widgets/widgets-core";

import type { UserCard } from "../../helpers/definitions.js";
import { Utils } from "../../showcases/table/utils.js";

const shouldPin = !DeviceDetector.isPhone() ? true : undefined;
export type RowType = UserCard & { id: number };

const ROW_COUNT = 500;

const columns: BaseColumnType<RowType>[] = [
	{ label: "ID", dataKey: "id", pinning: shouldPin && "left", width: 0.5 },
	{ label: "Name", dataKey: "name", pinning: shouldPin && "left", width: 1 },
	{
		label: (
	...
	{
		dataGetter: (): ReactNode => <Button title="Remove" icon={<Icon>remove_circle</Icon>} />,
		label: "",
		width: 0.5,
		pinning: "right"
	}
...
export function WithDndShowcase(): ReactElement {
	const DATA: RowType[] = useMemo(
		() => Utils.generateUserCardData(ROW_COUNT).map((card, index) => ({ ...card, id: index })),
		[]
	);
	...
	const dragDropOptions = useMemo<TableDragDropOptions<RowType>>(() => {
		return {
			onDrop: ({ dragItem, dropResult }) => {
				setData((data) => {
					...
					const newIndex = dropResult.rowIndex - (dragItem.rowIndex < dropResult.rowIndex ? 1 : 0);

					const [movedItem] = newData.splice(dragItem.rowIndex, 1);
