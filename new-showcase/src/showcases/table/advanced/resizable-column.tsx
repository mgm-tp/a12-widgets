...
function updateStates(params: {
	column: ColumnType;
	setSortState: Dispatch<SetStateAction<SortState<ColumnType>>>;
	sortState: SortState<ColumnType>;
	resizedWidthsGetter?: (column: ColumnType) => Column.Width | undefined;
}): ColumnType {
	const { column, sortState, setSortState, resizedWidthsGetter } = params;
	...
	const onEndResize: ColumnResizeEventHandler<ColumnType> = useCallback(
		({ resizedWidthsGetter }): void => {
			setColumns((oldColumns) =>
				oldColumns.map((column) => updateStates({ column, sortState, setSortState, resizedWidthsGetter }))
			);
		},
		[sortState]
	);
