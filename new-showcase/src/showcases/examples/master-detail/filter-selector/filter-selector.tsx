...
import type { Data, ListOperationType } from "./data.js";
import * as Utils from "./common.js";
import { FilterView } from "./filter-view.js";

export interface MasterDetailFilterSelectorProps {
	isSmallSize?: boolean;
	activeFilters: Data[];
...
export const MasterDetailFilterSelector: FC<MasterDetailFilterSelectorProps> = (props) => {
	...
	const { onApplyButtonClick, onCloseButtonClick, onVisibilityChange, isSmallSize } = props;

	const handleFilterToggle = useCallback(
		(id: string): void => {
			setActiveFilters((prevState) => Utils.toggleFilterInList(id, prevState));
			setInactiveFilters((prevState) => Utils.toggleFilterInList(id, prevState));

			const isFilterActive = Utils.isFilterActive(id, [...activeFilters, ...inactiveFilters]);
	...
	const getOptionData = useCallback((filterOptions: any, id: string, operation?: ListOperationType): void => {
		setActiveFilters((prevState) =>
			prevState.map((filter) =>
				filter.id === id
					? { ...filter, options: filterOptions, active: filterOptions !== "Inactive", operation }
					: filter
			)
		);
		setInactiveFilters((prevState) =>
			...
			(id?: string): ReactNode => {
				...
				return (
					<FilterView.View
						...
						getOptionData={(text, operation) => getOptionData(text, filterToRender.id, operation)}
						isMobile={isMobile}
					/>
				);
			},
		[activeFilters, getOptionData, inactiveFilters, isMobile]
	);
	...
	const getPrimaryContentProps = useMemo(
		() =>
			(
				...
				return {
					headingButtons: isSmallSize ? renderMobileButton([...activeFilters, ...inactiveFilters]) : undefined,
		...
		[activeFilters, inactiveFilters, isSmallSize, renderMobileButton]
	);
	...
	const inactiveFiltersWithSection = useMemo(
		() => Utils.getInactiveFiltersWithSections(searchParam, activeFilters, inactiveFilters),
		[activeFilters, inactiveFilters, searchParam]
	);
	const sortedActiveFilters = useMemo(
		() => (!searchParam.trim() && Utils.getSortedFilters(activeFilters)) || [],
		[activeFilters, searchParam]
	);

	return props.isSmallSize ? (
		<FilterSelectorMobile
