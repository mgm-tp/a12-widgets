...
export function useTreeNodeData(
	data: DragAndDrop.UiData,
	rootNodeId: number,
	position: DragAndDrop.Position
): DragAndDrop.TreeNode {
	function buildTree(nodeId: number): DragAndDrop.TreeNode | undefined {
		const node = data[nodeId];

		if (!node || !DragAndDrop.isTreeNode(node)) {
			return undefined;
		}
		...
		if (node.position.component === position.component && node.position.side === position.side) {
			return {
				...node,
				children: node.children?.map(buildTree).filter((node) => !!node)
			};
		}
	...
	if (!tree) {
		throw new Error(`Cannot build tree with root ID ${rootNodeId}`);
	}
...
export function updateTreeNode(
	data: DragAndDrop.UiData,
	nodeId: number,
	updater: (currentNode: DragAndDrop.TreeNode) => DragAndDrop.TreeNode
): DragAndDrop.UiData {
	const node = data[nodeId];
