...
export function AdditionalCustomizations(): ReactElement {
	...
	return (
		<ConfigurationView
			configuration={
				<Checkbox
					checked={isTextAligned}
					onChange={setIsTextAligned}
					label="Text aligned right"
					title="Text aligned right"
					fitToParent={false}
				/>
			}
		>
			<div>
				<TextField
					id={id}
					value={inputValue || "example value"}
					...
					suffixes={[
						<Button destructive icon={<Icon>close</Icon>} title="Clear" onClick={handleClearButtonClick} />,
						<Button icon={<Icon>keyboard_arrow_down</Icon>} title="More" />
					]}
					helperText={helperText}
					textAlignment={isTextAligned ? "right" : undefined}
					ariaDescribedby={`${id}-warning-tooltip counter`}
				/>
				<br />
				<TextField
