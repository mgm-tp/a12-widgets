...
export function HideSelectAllOption(): ReactElement {
	...
	const items = useMemo(() => {
		...
		return getItems(selectedIds).map((item) =>
			selectedItems.length >= 3 && !selectedIds.includes(item.id) ? { ...item, disabled: true } : item
		);
	}, [selectedItems]);

	const handleOnChange = useCallback((items: MultiselectProps.Item[]) => {
	...
	return (
		<div className="-u-width-full">
			<Multiselect
				id="multiselect-with-no-all-option"
