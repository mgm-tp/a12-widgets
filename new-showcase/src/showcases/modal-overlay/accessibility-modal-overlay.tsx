...
export function AccessibilityModalExample(): ReactElement {
	const [isOpen, setOpen] = useState<boolean>(false);

	const showModal = (): void => setOpen(true);
	const closeModal = (): void => setOpen(false);

	const headingTitleId = "container-header-title";
		...
		<div className="-u-width-full -u-flex -u-justify-center">
			...
			{isOpen && (
				<ModalOverlay
					closeOnOutsideClick
					onClose={closeModal}
					containerAttributes={{ "aria-labelledby": headingTitleId }}
				>
					<ActionContentbox
						headingElements={<ContentBoxElements.Title id={headingTitleId} text="Simple Modal" />}
						headingButtons={<ContentBoxElements.CloseButton onClick={closeModal} />}
					>
						<p>{loremIpsum({ units: "sentences", count: 20 })}</p>
					</ActionContentbox>
				</ModalOverlay>
			)}
		</div>
