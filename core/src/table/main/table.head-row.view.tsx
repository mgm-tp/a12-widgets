...
import { isColumnGroup } from "./table.utils.js";
import { useTableContext } from "./table.context.js";
import { RowSegments } from "./table.row-segments.view.js";
import { HeadGridRow } from "./head-grid-row.view.js";

/** @internal */
export const HeadRow = memo(function HeadRow(props: TableRenderPropsType.HeadRowProps) {
	...
	const cellRenderer: RowSegments.CellRenderer = useCallback(
		({ column, indexInRow }) => {
			const renderer = isColumnGroup(column) ? headCellGroupRenderer : headCellRenderer;

			return renderer({ column, key: indexInRow });
		},
		[headCellGroupRenderer, headCellRenderer]
	);

	// Use enhanced accessibility rendering when enabled
	if (enableColumnGroupA11y) {
		return <HeadGridRow {...props} />;
	}

	return (
		<TableTemplate.HeadRow {...props} role={cardView ? false : props.role}>
			<RowSegments SegmentComponent={TableTemplate.HeadRowSegment} cellRenderer={cellRenderer} />
		</TableTemplate.HeadRow>
	);
});
