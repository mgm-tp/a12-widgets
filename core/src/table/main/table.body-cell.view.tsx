...
import { StyledTableTemplate, TableTemplate } from "./template/index.js";
import { StyledBaseTable } from "./template/table.styled.js";
import { useStyledTableContext } from "./template/table.context.styled.js";
	...
	return !isSingleColumn ? (
		...
		>
			{column.subColumns?.map((col, index) => {
				...
				let subColAriaColIndex: number | undefined;

				if (enableColumnGroupA11y && flattenColumns) {
					const subColIndex = Utils.getColumnIndex(col, flattenColumns);

					if (subColIndex >= 0) {
						subColAriaColIndex = subColIndex + 1;
					}
				}
				...
				return bodyCellRenderer({
					...
					...(subColAriaColIndex !== undefined && { ariaColIndex: subColAriaColIndex })
				});
			})}
		</StyledTableBodyCellGroup>
	) : (
