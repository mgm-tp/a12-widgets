...
export const StyledTableBodyRowSegment = styled(StyledBaseTable.Segment).withConfig({
	...
	return css`
		...
		rowSegmentType === "left" &&
		css`
			${StyledTableBodyRow}:not(:last-child) && {
				// fix cannot right-click on body row to open context menu
				${createPseudoElement(
					":before",
					css`
		...
		&:last-child > :last-child {
			${StyledTableBodyRow}:focus & {
				...
				&:before {
					border-right: ${bodyRow.interactive.focusBorder};
					margin-right: 1px;
				}
			}
			@media (pointer: fine) {
				${StyledTableBodyRow}:not(:focus):hover & {
					&:before {
						border-right: ${bodyRow.interactive.hoverBorder};
					}
				}
			}
		}
...
export const BodyRowSegmentTpl = memo(function BodyRowSegmentTpl(
...
BodyRowSegmentTpl.displayName = "BodyRowSegmentTpl";
