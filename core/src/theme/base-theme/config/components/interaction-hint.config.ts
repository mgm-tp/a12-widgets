...
export type InteractionHintConfigType = {
	containerMaxWidth: string;
	arrow: { background: string; border: string; boxShadow: string; size: number };
	margin: string;
	border: string;
	content: {
		background: string;
...
const defaultInteractionHintConfig = (theme: BaseThemeCore): InteractionHintConfigType => {
	const spacing = theme.spacing;
	const colors = theme.colors;
	const typography = theme.typography;

	return {
		containerMaxWidth: "472px",
		...
		arrow: {
			...
			size: spacing.spacing.spacingSm
		},
...
export const interactionHintOverrides = (theme: BaseThemeCore) => {
	const colors = theme.colors;
...
export const interactionHintConfig = (theme: BaseThemeCore) =>
	mergeConfig(defaultInteractionHintConfig(theme), interactionHintOverrides(theme));
