...
const defaultDateTimePickerConfig = (theme: BaseThemeCore): DateTimePickerConfigType => {
	...
	return {
		dateScreen: {
			...
			datePicker: {
				body: {
					padding: `0 ${spacing.horizontalSpacing.horizWhiteSpacingsm}px ${spacing.verticalSpacing.vertWhiteSpacing2xs}px`
				},
				weekDay: {
...
export const dateTimePickerOverrides = (theme: BaseThemeCore) => {
	const colors = theme.colors;
	const focusStyles = theme.focusStyles;
	const buttonConfigs = commonButtonConfigs(theme);
...
export const dateTimePickerConfig = (theme: BaseThemeCore) =>
	mergeConfig(defaultDateTimePickerConfig(theme), dateTimePickerOverrides(theme));
