...
export const switchConfig = (theme: BaseThemeCore): SwitchConfigType => {
	...
	return {
		...
		optionLabel: {
			...
			checkedMargin: `0 0 0 ${spacing.horizontalSpacing.horizWhiteSpacingxs}px`,
			fontFamily: typography.font.MAIN_FONT,
			fontSize: typography.fontSize.tinyFontSize,
			uncheckedMargin: `0 ${spacing.horizontalSpacing.horizWhiteSpacingxs}px 0 0`,
		...
		thumb: {
			...
			focus: {
				...
				size: `${spacing.spacing.spacingMd + spacing.spacing.spacing3xs}px`
			},
			hover: {
		...
		track: {
			border: `${theme.border.width.thin} dotted ${colors.graphicSecondaryColorDark}`,
			disabled: {
			...
			width: `${spacing.horizontalSpacing.horizWhiteSpacingxl}px`
		},
