...
export type RichTextEditorConfigType = {
	toolbar: {
		background: string;
		borderRadius: string;
		boxShadow: string;
...
const defaultRichTextEditorConfig = (theme: BaseThemeCore): RichTextEditorConfigType => {
	...
	return {
		toolbar: {
			...
			item: {
				...
				width: 2.5 * spacing.baseSpacing.BASE + "px"
			},
			separator: {
...
export const richTextEditorOverrides = (theme: BaseThemeCore) => {
	const colors = theme.colors;
...
export const richTextEditorConfig = (theme: BaseThemeCore) =>
	mergeConfig(defaultRichTextEditorConfig(theme), richTextEditorOverrides(theme));
