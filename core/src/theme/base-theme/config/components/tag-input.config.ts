...
const defaultTagInputConfig = (theme: BaseThemeCore): TagInputConfigType => {
...
export const tagInputOverrides = (theme: BaseThemeCore) => {
	const colors = theme.colors;

	return {
		tag: {
			activeBorderColor: colors.interaction.color,
			hoverBorderColor: colors.interaction.color
		},
		textArea: {
			border: `1px dashed ${colors.divider.colorDark}`
		}
	};
};

export const tagInputConfig = (theme: BaseThemeCore) =>
	mergeConfig(defaultTagInputConfig(theme), tagInputOverrides(theme));
