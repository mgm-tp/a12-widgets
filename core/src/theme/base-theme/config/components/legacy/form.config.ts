...
/**
 * @deprecated Legacy Form component config. Form styling is now handled by individual field components.
 */
...
const defaultFormConfig = (theme: BaseThemeCore): FormConfigType => {
	...
	return {
		...
		section: {
			contentTitle: {
				...
				borderBottom: `${theme.border.width.medium} solid ${colors.primaryColor}`,
...
export const formOverrides = (theme: BaseThemeCore) => {
	...
	return {
		...
		collapsiblePanel: {
			h3: {
				...
				color: themeColors.text.titleColor,
...
export const formConfig = (theme: BaseThemeCore) => mergeConfig(defaultFormConfig(theme), formOverrides(theme));
