...
const { StyledSelectInput, StyledFieldSelectControl } = StyledSelectTemplate;
const { StyledFieldGroup, StyledFieldControl } = StyledBaseBoolean;

export const baseFormClassName = addPrefix("form");

export const backgroundColorHelper = css`
	.${addPrefix("h")} {
		${() => {
			...
			for (const key in secondaryColors) {
				const bgValue = secondaryColors[key as keyof typeof secondaryColors];
				styles.push(css`
					&_${key}BG {
						&${StyledButton} {
						...
						> ${StyledFieldWrapper},
							...
							${StyledSelectInput}:not(:disabled), 
