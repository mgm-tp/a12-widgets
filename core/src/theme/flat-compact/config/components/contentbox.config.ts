...
export const contentBoxFlatCompactConfig = (theme: FlatCompactThemeType) => {
	const { colors, spacing, typography } = theme;
	const contentBoxHorizSpacing = `${spacing.horizontalSpacing.horizWhiteSpacingmd}px`;

	return {
		heading: { paddingLeft: contentBoxHorizSpacing, paddingRight: contentBoxHorizSpacing },
		actionBar: {
			minHeight: `${2 * spacing.spacing.spacingMd + 4}px`,
			padding: `${spacing.verticalSpacing.vertWhiteSpacing3xs}px ${contentBoxHorizSpacing}`
		},
		content: {
			padding: `${spacing.verticalSpacing.vertWhiteSpacingmd}px ${contentBoxHorizSpacing}`
		},
		...
		title: {
			fontSize: `calc(1.1 * ${typography.fontSize.mediumFontSize})`
		},
		tile: {
			icon: {
				fontSize: theme.typography.fontSize.hugeFontSize
			}
		},
