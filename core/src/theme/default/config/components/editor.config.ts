...
export const editorConfig = (theme: BaseThemeType): EditorConfigType => {
	...
	return {
		...
		mention: {
			...
			suggestions: {
				...
				borderRadius: "2px",
				boxShadow: `0 4px 2px 0 ${rgba(colors.boxShadowBackground, 0.4)}`,
				maxWidth: 5 * spacing.spacing.spacing2Xl + "px",
				minWidth: 2.5 * spacing.spacing.spacing2Xl + "px"
			}
		},
		misspelledWord: {
			activeBG: rgba(colors.variant.errorColor, 0.5),
			hoverBG: rgba(colors.variant.errorColor, 0.5)
		},
		singleLine: {
			contentMargin: "0"
		},
		...
		tooltip: {
			...
			borderRadius: "2px",
			boxShadow: `1px 2px 2px 0 ${rgba(colors.boxShadowBackground, 0.4)}`
		}
	};
};
