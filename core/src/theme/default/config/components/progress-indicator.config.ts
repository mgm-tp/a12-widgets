...
export const progressIndicatorConfig = (theme: BaseThemeType): ProgressIndicatorConfigType => {
	const { colors, spacing, typography } = theme;

	return {
		brightBG: rgba(colors.background.secondaryBackground, 0.9),
		circle: {
			animation: {
				duration: "0.4s",
		...
		innerOverlay: {
			...
			background: rgba(GeneralColorsConfig.black, 0.8),
			borderRadius: "2px",
			boxShadow: `0 0 2px ${GeneralColorsConfig.black}`,
		...
		outerOverlay: {
			...
			background: rgba(colors.boxShadowBackground, 0.4)
		}
	};
};
