...
export const dropdownConfig = (theme: BaseThemeType): DropdownConfigType => {
	const { colors, spacing, typography, applicationStyles, focusStyles } = theme;

	return {
		background: colors.background.interactiveBackground,
		boxShadow: `0 1px 2px 0 ${rgba(colors.boxShadowBackground, 0.4)}`,
		outline: `1px solid ${colors.divider.colorSubtle}`,
		contentMaxHeight: `${3 * spacing.spacing.spacing2Xl}px`,
		fontFamily: typography.font.MAIN_FONT,
		graphic: {
			lineHeight: `${spacing.baseSpacing.BASE}px`,
		...
		item: {
			...
			focusBorder: `1px dotted ${colors.text.headlineColor}`,
			focusPreselectBorder: `2px solid ${colors.interaction.selected.color}`,
		...
		link: {
			...
			wrapper: {
				borderBottom: `1px solid ${colors.divider.color}`,
				touchBorderBottom: `1px solid ${colors.divider.colorDark}`
			}
		},
		secondaryText: {
