...
export type AutocompleteConfigType = {
	dropdown: { loadingMinHeight: string; margin: string };
	input?: { focus?: { customBorder?: CustomBorder } };
};

export const autocompleteConfig = (theme: BaseThemeType): AutocompleteConfigType => {
	const { spacing } = theme;

	return {
		dropdown: {
			loadingMinHeight: `${2 * spacing.spacing.spacingMd}px`,
