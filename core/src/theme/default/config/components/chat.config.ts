...
export const chatConfig = (theme: BaseThemeType): ChatConfigType => {
	const {
		colors,
		spacing: { horizontalSpacing, verticalSpacing, spacing },
		typography: { fontSize, font, fontWeight }
	} = theme;
	const chatUserAvatarSize = fontSize.mediumFontSize;
	...
	return {
		...
		message: {
			...
			borderColor: {
				left: GeneralColorsConfig.blueLight,
				right: colors.background.secondaryBackground
			},
			wrapper: {
			...
			content: {
				...
				lineHeight: fontSize.lgFontSize
			},
			status: {
				fontFamily: font.MAIN_FONT,
		...
		userInfo: {
			...
			name: {
				...
				lineHeight: fontSize.mediumFontSize
			}
		},
		date: {
			marker: {
