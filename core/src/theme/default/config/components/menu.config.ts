...
const commonItemConfigs = (theme: BaseThemeType) => {
	const { colors, focusStyles } = theme;

	return {
		active: {
			background: "transparent",
			border: "none",
			borderBottom: `4px solid ${colors.divider.colorLight}`,
			borderRadius: "0",
...
export const menuConfig = (theme: BaseThemeType): MenuConfigType => {
	const { colors, focusStyles, spacing, typography, divisionLineStyles } = theme;
	...
	return {
		...
		item: {
			...
			subHorizontal: {
				...
				selected: {
					activeBorderLeft: `4px solid ${colors.interaction.active.colorTouchInverted}`,
					background: "rgba(0,0,0,0.4)",
					borderLeft: `4px solid ${colors.interaction.selected.colorInverted}`,
					textColor: colors.text.invertedColor,
					focusBorderLeft: `4px solid ${colors.interaction.focus.colorInverted}`,
					hoverBorderLeft: `4px solid ${colors.interaction.focus.colorInverted}`,
					hover: {
						borderBottom: `4px solid ${colors.divider.colorLight}`,
						color: colors.text.invertedColor,
						cursor: "pointer"
