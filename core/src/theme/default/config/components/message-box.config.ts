...
export const messageBoxConfig = (theme: BaseThemeType): MessageBoxConfigType => {
	const {
	...
	return {
		border: {
			error: `2px solid ${colors.variant.errorColor}`,
			info: `2px solid ${colors.variant.infoColor}`,
			success: `2px solid ${colors.variant.successColor}`,
			warning: `2px solid ${colors.variant.warningColor}`
		},
		background: {
		...
		response: {
			topBorder: `2px solid ${colors.divider.color}`,
