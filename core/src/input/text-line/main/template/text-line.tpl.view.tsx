...
import type { TextAffixProps, TextLineStatelessProps } from "./text-line.tpl.api.js";
...
export function TextLineStateless(
	props: TextLineStatelessProps & {
		/**
	...
	const realInput = useMemo((): ReactNode => {
		...
		return (
			<StyledBaseInput.StyledFieldTextInput
				...
				data-role={DataRoles.Textline.Input}
		...
		return (
			...
			prefixesArray.map((prefix, index) => (
				<StyledBaseInput.StyledFieldPrefixWrapper
					data-role={`${DataRoles.Textline.Prefix}-${index}`}
		...
		return (
			...
			suffixesArray.map((suffix, index) => (
				<StyledBaseInput.StyledFieldSuffixWrapper
					...
					$first={index === 0}
					key={`textline-suffix-${index}`}
					data-role={`${DataRoles.Textline.Suffix}-${index}`}
					onMouseEnter={addNoEffectClass}
					onMouseLeave={removeNoEffectClass}
					onTouchStart={addNoEffectClass}
					onTouchEnd={removeNoEffectClass}
			...
			return (
				...
				addons.map((addon, index) => (
					<StyledBaseInput.StyledFieldAddon
						...
						key={`textline--addon-before-${index}`}
						data-role={belongsToMain ? `textline--addon-before-${index}` : undefined}
					>
			...
			return (
				...
				addons.map((addon, index) => (
					<StyledBaseInput.StyledFieldAddon
						...
						key={`textline--addon-after-${index}`}
						data-role={belongsToMain ? `textline--addon-after-${index}` : undefined}
					>
						{belongsToMain || !isValidElement<Identifiable>(addon) ? addon : cloneElement(addon, { id: undefined })}
					</StyledBaseInput.StyledFieldAddon>
				))
	...
	const labelElement = useMemo((): ReactNode => {
		...
		return (
			<InputElements.Label
				...
				htmlFor={id}
				dataRole={DataRoles.Textline.Label}
				label={labelEl}
		...
		>
			<StyledBaseInput.StyledFieldMain className={`${baseFieldClassName}__main`}>
				...
				<StyledBaseInput.StyledField
					...
					data-role={DataRoles.Textline.Control}
					role={role}
				>
					{labelElement}
					{tooltipElement}
					{errorMessage && (
						<InputElements.Error
							...
							id={id}
							dataRole={DataRoles.Textline.ErrorMessage}
					...
					{warningMessage && (
						<InputElements.Warning
							className={`${baseFieldClassName}__message`}
							id={id}
							dataRole={DataRoles.Textline.WarningMessage}
					...
					{infoMessage && (
						<InputElements.Info
							className={`${baseFieldClassName}__message`}
							id={id}
							dataRole={DataRoles.Textline.InfoMessage}
					...
					<StyledBaseInput.StyledFieldInput
						...
						data-role={DataRoles.Textline.Input.Wrapper}
			...
			{helperText && (
				<StyledBaseInput.StyledFieldHelperWrapper className={`${baseFieldClassName}__helper`} ref={helperTextRef}>
					...
					<StyledBaseInput.StyledFieldHelperText
						...
						htmlFor={id}
						data-role={DataRoles.Textline.HelperText}
					>
						{helperText}
					</StyledBaseInput.StyledFieldHelperText>
...
TextLineStateless.displayName = "TextLineStateless";
...
export function TextAffix(props: TextAffixProps): ReactElement<TextAffixProps> {
	return (
		<StyledBaseInput.StyledFieldAffixText
			...
			data-role={DataRoles.Textline.TextAffix}
			key="textline-text-affix"
			className={joinClassNames(
