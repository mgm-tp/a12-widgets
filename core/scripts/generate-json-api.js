...
const app = await Application.bootstrapWithPlugins({
	tsconfig: Path.join(coreDir, "tsconfig.build.json"),
	basePath: toPosix(srcDir),
	entryPoints: [toPosix(srcDir)],
	entryPointStrategy: "expand",
	exclude: ["**/*+(.test|index|internal|styled).ts", "**/*+(.view|test|internal|styled).tsx"],
...
const typedoc = app.serializer.projectToObject(project, toPosix(projectDir));

for (const module of typedoc.children ?? []) {
