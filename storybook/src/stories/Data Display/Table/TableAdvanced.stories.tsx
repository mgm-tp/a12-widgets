...
function VirtualizedDragAndDropDemo() {
	const [data, setData] = useState<DataRow[]>(VIRTUAL_ROWS);

	const onDrop = useCallback<NonNullable<TableDragDropOptions<DataRow>["onDrop"]>>(({ dragItem, dropResult }) => {
		setData((prev) => {
			const next = [...prev];
			const moved = next[dragItem.rowIndex];
...
export const VirtualizedDragAndDrop: Story = {
	...
	parameters: {
		docs: {
			description: {
				story:
					"Combine `dragDropOptions` with `virtualScrollOptions` to reorder rows in a windowed table. This table renders 500 rows but keeps only the visible window mounted; every rendered row stays draggable, and dragging toward the top/bottom edge auto-scrolls to reveal off-window drop targets. The container needs an explicit height."
			}
		}
	}
};
